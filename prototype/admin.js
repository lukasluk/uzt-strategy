        ...
        ${state.guidelines.map((guideline) => {
          ...
          <article class="card ${guideline.status === 'active' ? '' : 'admin-panel'}">
            ...
            <div class="admin-comment-box">
              ...
              ` : '<p class="prompt">Komentar� dar n�ra.</p>'}
            </div>
          </article>
        `;
        }).join('')}
    ...
    button.addEventListener('click', async () => {
      ...
      await runBusy(async () => {
        ...
        state.notice = nextStatus === 'hidden'
          ? 'Komentaras pasl�ptas.'
          : 'Komentaras v�l rodomas.';
        await bootstrap();
      });
    });
  });
