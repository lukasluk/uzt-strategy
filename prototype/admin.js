...
function resolveEmbedLanguage(){const fromI18n=String(window.DigiI18n?.getLanguage?.()||'').trim().toLowerCase();if(fromI18n)return fromI18n==='en'?'en':'lt';const fromHtml=String(document.documentElement?.lang||'').trim().toLowerCase();return fromHtml==='en'?'en':'lt';}
function buildInstitutionEmbedSourceUrl(){const slug=normalizeSlug(state.institutionSlug);if(!slug)return'';const lang=resolveEmbedLanguage();const base=`${window.location.origin}${EMBED_MAP_PATH_PREFIX}/`;return `${base}?institution=${encodeURIComponent(slug)}&${EMBED_QUERY_KEY}=${EMBED_MAP_VALUE}&view=map&lang=${encodeURIComponent(lang)}`;}
function buildInstitutionEmbedIframeCode(){const src=buildInstitutionEmbedSourceUrl();if(!src)return'';return['<iframe',`  src="${src}"`,'  width="100%"','  height="720"','  loading="lazy"','  style="border:0;overflow:hidden;"','  referrerpolicy="strict-origin-when-cross-origin"','  allowfullscreen','></iframe>'].join('\n');}
function renderCycleEmbedCard(embedCode){if(!embedCode)return'';return `<section class="card admin-embed-card" data-admin-section="cycle" style="margin-bottom:16px;"><div class="header-row"><strong>Embed: Strategijų žemėlapis (view-only)</strong><span class="tag">Institucija: ${escapeHtml(state.institutionSlug)}</span></div><p class="prompt admin-embed-help">Įdėkite kodą į savo svetainę. Įterptas blokas rodo tik viešus pasirinktos institucijos žemėlapio duomenis.</p><textarea id="adminEmbedCode" class="admin-embed-code" readonly>${escapeHtml(embedCode)}</textarea><div class="inline-form" style="margin-top:10px;"><button id="copyEmbedCodeBtn" class="btn btn-primary" type="button">Kopijuoti embed kodą</button></div></section>`;}
...
function renderDashboard(){
  ...
  const cycleState=String(cycle?.state||'open');
  const embedCode=buildInstitutionEmbedIframeCode();
  root.innerHTML=`
    ${state.error?`<section class="card" style="margin-bottom:16px;"><strong>Klaida</strong><p class="prompt">${escapeHtml(state.error)}</p><button id="retryBtn" class="btn btn-primary">Bandyti dar kartą</button></section>`:''}
    ${state.notice?`<section class="card" style="margin-bottom:16px;"><strong>${escapeHtml(state.notice)}</strong></section>`:''}
    <section class="card admin-tabs-card" style="margin-bottom:16px;">
      <div class="admin-tabs">
        <button type="button" class="admin-tab-btn ${state.adminTab==='cycle'?'active':''}" data-admin-tab="cycle">Bendri</button>
    ...
    <section class="card" data-admin-section="cycle" style="margin-bottom:16px;">
      ...
      <p class="prompt">Būsena valdo, ar nariai gali balsuoti ir komentuoti (OPEN). Uždarius ciklą (CLOSED), balsavimas ir komentavimas išjungiami.</p>
      <div class="inline-form">
        <select id="cycleStateSelect" ${state.busy||!cycle?.id?'disabled':''}>${['open','closed'].map(i=>`<option value="${i}" ${i===cycleState?'selected':''}>${i.toUpperCase()}</option>`).join('')}</select>
      ...
      <div class="admin-edit" style="margin-top:12px;">
        ...
        <div class="admin-add-form">
          ...
          <textarea id="cycleVisionInput" class="admin-edit-description" placeholder="Vizija" ${state.busy||!cycle?.id?'disabled':''}>${escapeHtml(cycle?.vision_text||'')}</textarea>
          <button id="saveCycleNarrativeBtn" class="btn btn-primary" type="button" ${state.busy||!cycle?.id?'disabled':''}>Išsaugoti misiją ir viziją</button>
        </div>
      </div>
    </section>
    ${renderCycleEmbedCard(embedCode)}
    <section class="card" data-admin-section="users" style="margin-bottom:16px;">
      <div class="header-row"><strong>Kvietimai nariams</strong><span class="tag">Invite-only</span></div>
      ...
      ${state.inviteToken?`<div class="card" style="margin-top:12px;"><strong>Naujas kvietimo žetonas</strong><p class="prompt" style="word-break:break-all;">${escapeHtml(state.inviteToken)}</p><button id="copyInviteTokenBtn" class="btn btn-ghost">Kopijuoti žetoną</button></div>`:''}
    </section>
    <section class="card" data-admin-section="users" style="margin-bottom:16px;">
      <div class="header-row"><strong>Dalyviai</strong><span class="tag">${state.participants.length}</span></div>
      <ul class="mini-list">${state.participants.length?state.participants.map(p=>`<li><div class="header-row" style="align-items:flex-start;gap:8px;"><div><strong>${escapeHtml(p.display_name||p.email)}</strong><div class="prompt" style="margin-top:4px;">El. paštas: ${escapeHtml(p.email||'-')}</div></div><div class="header-stack" style="margin-left:auto;"><span class="muted">${p.has_voted?'Balsavo':'Nebalsavo'}</span><span class="tag">Taškai: ${Number(p.total_score||0)}</span></div></div><div class="inline-form" style="margin-top:8px;"><form class="participant-password-form inline-form" data-user-id="${escapeHtml(p.id)}"><input type="password" name="password" placeholder="Naujas slaptažodis (min. 8)" minlength="8" required ${state.busy?'disabled':''}/><button type="submit" class="btn btn-ghost" ${state.busy?'disabled':''}>Atnaujinti slaptažodį</button></form><button type="button" class="btn btn-ghost participant-delete-btn" data-user-id="${escapeHtml(p.id)}" data-name="${escapeHtml(p.display_name||p.email||'vartotojas')}" ${state.busy?'disabled':''}>Ištrinti vartotoją</button></div></li>`).join(''):'<li>Nėra dalyvių.</li>'}</ul>
    </section>
    <section class="card" data-admin-section="guidelines" style="margin-bottom:16px;">
      <div class="header-row"><strong>Pridėti gaires</strong><span class="tag">${state.guidelines.length} viso</span></div>
      <form id="addGuidelineForm" class="admin-add-form">
        <div class="form-row"><input type="text" name="title" placeholder="Gairės pavadinimas" required ${state.busy||!cycle?.id?'disabled':''}/></div>
        <textarea class="admin-edit-description" name="description" placeholder="Trumpas paaiškinimas" ${state.busy||!cycle?.id?'disabled':''}></textarea>
        <button class="btn btn-primary" type="submit" style="margin-top:12px;" ${state.busy||!cycle?.id?'disabled':''}>Pridėti gairę</button>
      </form>
    </section>
    <section class="card" data-admin-section="guidelines" style="margin-bottom:16px;">
      <div class="header-row"><strong>Gairių redagavimas ir komentarų moderavimas</strong><span class="tag">${state.guidelines.length}</span></div>
      <div class="card-list admin-guideline-grid">${renderGuidelineCards()}</div>
    </section>
    <section class="card" data-admin-section="initiatives" style="margin-bottom:16px;">
      <div class="header-row"><strong>Pridėti iniciatyvą</strong><span class="tag">${state.initiatives.length} viso</span></div>
      <form id="addInitiativeForm" class="admin-add-form">
        <div class="form-row"><input type="text" name="title" placeholder="Iniciatyvos pavadinimas" required ${state.busy||!cycle?.id?'disabled':''}/></div>
        <textarea class="admin-edit-description" name="description" placeholder="Trumpas paaiškinimas" ${state.busy||!cycle?.id?'disabled':''}></textarea>
        <label class="prompt" style="display:block;margin:8px 0 6px;">Priskirtos gairės (bent 1)</label>
        <select name="guidelineIds" multiple size="${Math.min(Math.max(state.availableGuidelines.length,4),10)}" ${state.busy||!cycle?.id?'disabled':''}>${guidelineOptions([])}</select>
        <div class="inline-form" style="margin-top:12px;">
          <select name="lineSide" ${state.busy||!cycle?.id?'disabled':''}>
            <option value="auto">Linija: auto</option>
            <option value="left">Linija: kairė</option>
            <option value="right">Linija: dešinė</option>
            <option value="top">Linija: viršus</option>
            <option value="bottom">Linija: apačia</option>
          </select>
          <button class="btn btn-primary" type="submit" ${state.busy||!cycle?.id?'disabled':''}>Pridėti iniciatyvą</button>
        </div>
      </form>
    </section>
    <section class="card" data-admin-section="initiatives" style="margin-bottom:16px;">
      <div class="header-row"><strong>Iniciatyvų redagavimas ir komentarų moderavimas</strong><span class="tag">${state.initiatives.length}</span></div>
      <div class="card-list admin-guideline-grid admin-initiative-grid">${renderInitiativeCards()}</div>
    </section>
  `;
  applyTabVisibility();
  bindDashboardEvents();
...
const saveCycleNarrativeBtn=document.getElementById('saveCycleNarrativeBtn');if(saveCycleNarrativeBtn){saveCycleNarrativeBtn.addEventListener('click',async()=>{if(!state.cycle?.id)return;const missionText=String(document.getElementById('cycleMissionInput')?.value||'').trim();const visionText=String(document.getElementById('cycleVisionInput')?.value||'').trim();await runBusy(async()=>{await api(`/api/v1/admin/cycles/${encodeURIComponent(state.cycle.id)}/settings`,{method:'PUT',body:{missionText,visionText}});state.notice='Misija ir vizija išsaugotos.';await bootstrap();});});}
const copyEmbedCodeBtn=document.getElementById('copyEmbedCodeBtn');const adminEmbedCode=document.getElementById('adminEmbedCode');if(copyEmbedCodeBtn&&adminEmbedCode){copyEmbedCodeBtn.addEventListener('click',async()=>{const value=String(adminEmbedCode.value||'');if(!value)return;try{await navigator.clipboard.writeText(value);}catch{adminEmbedCode.focus();adminEmbedCode.select();document.execCommand('copy');}state.notice='Embed kodas nukopijuotas.';render();});}
